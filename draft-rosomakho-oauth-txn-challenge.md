...
In the relay profile the grant request carries no proof of possession, and the authorization server returns a
transaction token rather than a sender-constrained access token. For example, the grant request is:
...
~~~
{: #fig-relay-request title="Relay-profile transaction authorization grant request"}

On approval, the resolving token response is a transaction token:
...
{
  "access_token": "eyJhbGciOiJFUzI1NiIsInR5cCI6InR4bl90b2tlbitqd3QifQ...",
  "issued_token_type": "urn:ietf:params:oauth:token-type:txn_token",
  "token_type": "N_A"
}
~~~
{: #fig-relay-response title="Relay-profile transaction token response"}

The client presents the transaction token to the protected resource in the `Txn-Token` header field defined by
{{TXN-TOKENS}}, alongside the access token it already uses for the protected resource.
...
This appendix shows a complete key-bound flow for the running payment example, with the message bodies given in
the body of this document. It is non-normative. The agent holds a DPoP-bound access token for
`resource.example.com` and the protected resource trusts `as.example.com` to authorize payments.

1. The agent attempts the operation, advertising support and proving possession of its key:
   `POST /payments` with `Authorization: DPoP <at>`, `Accept-Txn-Challenge: ?1`, and a `DPoP` proof.

2. The protected resource requires transaction authorization. It records the JWK Thumbprint from the DPoP proof
   and returns `401` with `WWW-Authenticate: DPoP error="transaction_authorization_required",
   transaction_challenge="..."` (see {{fig-challenge-response}}). The challenge binds `cnf` to the agent key,
   names `aud=https://as.example.com`, carries `txn=97053963-...`, and describes the payment in
   `authorization_details` (see {{fig-challenge-example}}).

3. The agent validates the challenge, confirms `cnf` matches its key, and submits it to the token endpoint as the
   transaction authorization grant with `completion_mode=deferred` and a DPoP proof (see
   {{fig-transaction-authorization-request}}).

4. Approval is asynchronous, so the authorization server returns a deferred token response: `400` with
   `error=authorization_pending`, `deferral_token=8d67dc78-...`, and `interval` (see {{fig-deferred-response}}).
   The deferral token is bound to the agent key. The authorization server obtains approval from the approving
   party out of band.

5. The agent polls the token endpoint with `grant_type=urn:ietf:params:oauth:grant-type:deferred` and the
   `deferral_token`, signing each poll with the same key (see {{fig-deferred-poll}}), receiving
   `authorization_pending` until approval completes.

6. On approval, the poll returns a sender-constrained access token bound to the agent key (`cnf`), narrowed by
   the `authorization_details` and `txn` of the challenge (see {{fig-access-token-response}}).

7. The agent retries the operation, presenting the access token with `Authorization: DPoP <token>` and a DPoP
   proof (see {{fig-access-token-presentation}}). The protected resource validates the token, confirms proof of
   possession of the bound key, checks `txn`, and verifies that the payment it is about to make is within the
   token's `authorization_details`. It then processes the payment.
